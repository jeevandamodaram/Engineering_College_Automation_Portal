...
$ssql = "SELECT s1 FROM bsi  WHERE sem= '2' ";
...
$fsql = "SELECT fname FROM bfaculty  WHERE qsi='$sub' ";
...
.myTable th {height:50px;border:1px solid #ac8f57; background-color:#85144b;color:#fff }
.myTable td {padding:5px;border:1px solid #ac8f57; }
</style>
	</head>
...
//echo "$s";
echo "<form  method='post' action='succes.php?stid=". $qrow['stid'] ."   &    rno=" . $qrow['rno'] ." & sub= ".$s1."  & pg= ".$page." & pid= ".$pid." & fmid=".$m." '>";
...
echo " <td width='10%'>";
echo " <input class='minput' name='marks'  type='number' min=1 max=30 maxlength='2' placeholder=". $qrow["s1"]. "  required style='font-weight: bold; font-family: Century Gothic; color: #85144b' /></td>";

echo "<td  width='10%' >  <button class='button1'> Update </button> </td>";

echo "<td  width='10%' ><p align='center' style='font-size:20px; color:#85144b'>" . $qrow["s1"].  " </p> </td>";
